...
?>
<div class="jtcs_item_wrapper jt-cs himax solution" style="padding:<?php echo $params->get('content_padding');?>;">
<?php if( $params->get('show_pretext')==1 ): ?>
<div class="jt-pretext Animations fadeInUp2">
...
<?php 
	...
	foreach ($list as $i =>  $item) : 
		// Check whether thumbnail image is exist or not. If it's not then start thumbnail generation process
			...
			?>
			 <div class="slide wow fadeInUp" data-wow-duration="1s" data-wow-delay="<?php $increment = 0.2; echo (0.1 + ($i * $increment)); ?>s" style="padding:<?php echo $params->get('article_block_padding');?>;margin:<?php echo $params->get('article_block_margin');?>"  data-slide-index="<?php echo $i; ?>">		
			  <div class="jt-inner container-bg rounded h-100 p-4"><?php if($params->get('use_caption')== 1 &&  !empty($caption_text) ):?>
			  <div class="icon-title d-flex align-items-center  container-bg mb-4">
                           <span class="jt-caption"><i class="<?php echo $caption_text; ?>" aria-hidden="true"></i></span>
				...
				<?php if($params->get('showReadmore')== 1):?>
					<p class="jt-readmore">
						<a class="btn btn-primary jt-readmore" target="<?php echo $openTarget; ?>"
							...
							href="<?php echo $item->link;?>"><i class="<?php echo $params->get('ReadMoreIcon'); ?>">&nbsp;</i> <?php echo $params->get('ReadMoreText');?> <i class="fa-solid fa-chevron-right indented"> </i>
						</a>
					</p>
					<?php endif; ?>
