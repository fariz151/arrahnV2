...
class AngieModelSteps extends AModel
{
	...
	public function getSteps()
	{
		...
		// Try fetching the steps from the session
	...
	public function initialiseSteps()
	{
		...
		// This is necessary because this model is called very early in the startup sequence.
		AngieHelperPublicfolder::setUpInSession();

		// Check for Joomla 5+ custom public folder.
		if (strtolower(ANGIE_INSTALLER_NAME) === 'joomla' && in_array('JPATH_PUBLIC', $offsitedirs))
		{
			// Rearrange steps order to inject custom public folder before the offsitedirs step
			$savedSteps = $this->steps;

			$this->steps = [];

			$offsitedirs = $offsiteModel->getDirs(true, true);

			$publicFolder = [
				'source'    => APATH_SITE . '/' . $offsitedirs['JPATH_PUBLIC']['virtual'],
				'oldPublic' => $offsitedirs['JPATH_PUBLIC']['target'],
				'oldRoot'   => AngieHelperPublicfolder::getOldSiteRoot(),
				'newRoot'   => APATH_SITE
			];
			$this->container->session->set('joomla.angie_public_folder', $publicFolder);

			unset($offsitedirs['JPATH_PUBLIC']);

			$this->steps['main']         = $savedSteps['main'];
			$this->steps['database']     = $savedSteps['database'];
			$this->steps['publicfolder'] = null;

			if (!empty($offsitedirs))
			{
				$this->steps['offsitedirs']  = array_keys($offsitedirs);
			}
		...
		{
			$this->container->session->set('joomla.custom_public_folder', false);
		}
